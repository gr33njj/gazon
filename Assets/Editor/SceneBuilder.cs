    ...
    public static class SceneBuilder
    {
        ...
        private static void BuildShelfUnit(Transform parent, string letter, Vector3 position)
        {
            ...
            if (visual != null)
            {
                ...
                // (0.87, 2.44, 2.13) — по рангу размеров "родная" длинная ось модели —
                // её local Y (2.44, самая большая), высота — local Z (2.13), глубина —
                // local X (0.87, самая маленькая). ВАЖНО: сначала растягиваем МОДЕЛЬ В ЕЁ
                // РОДНОЙ (неповёрнутой) системе координат по этим самым осям, и только
                // ПОТОМ поворачиваем — localScale растягивает вдоль СОБСТВЕННЫХ локальных
                // осей объекта; если сделать наоборот (сначала повернуть, потом растянуть,
                // как было раньше), растяжение уходит не в мировые X/Y/Z, а в уже повёрнутые
                // локальные — стеллаж в итоге "ложится горизонтально". Порядок здесь и есть
                // сам фикс, ротация (ShelfModelRotation) как гипотеза не менялась.

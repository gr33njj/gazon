...
## Как собрать сцену — одной кнопкой (рекомендуется)

`Assets/Editor/SceneBuilder.cs` собирает всё ниже по коду через `UnityEditor` API (то есть настоящими
вызовами Unity, а не руками) — то же самое, что описано в ручном чек-листе, но одним кликом:

1. **File → New Scene** (важно начать с пустой сцены, иначе объекты задвоятся).
2. В меню редактора: **Gazon → Собрать сцену вертикального среза**.
3. Сохраните сцену (**Ctrl+S** / **Cmd+S**), дайте ей имя.
4. Убедитесь, что **Edit → Project Settings → Player → Other Settings → Active Input Handling**
   стоит в `Both` или `Input Manager (Old)` (это отдельная настройка проекта, скрипт её не трогает).
5. Нажмите **Play**.

Если запустить скрипт повторно на той же сцене — он остановится с ошибкой в Console («GameManager уже
есть на сцене»), чтобы не задвоить объекты; создайте новую пустую сцену для повторной попытки.

## Как собрать сцену руками (альтернатива, если хочется понять из чего она состоит)
